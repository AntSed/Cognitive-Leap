// app\components\games\GlyphRise.vue
   ...
   <v-stage :config="stageConfig" ref="stageRef">
    ...
    <v-layer v-if="gameEnded">
      ...
      <v-sprite
       ...
       :config="{
        x: stageConfig.width / 2 - 32,
        y: stageConfig.height / 2 - 160,
        width: 64,
        height: 64,
        image: princessImage,
...
const gameOverSpriteRef = ref(null)

const princessAnimations = {
  // 2-й ряд, 3 последних кадра (Веселая)
  'happy_idle': [384, 64, 64, 64, 448, 64, 64, 64, 512, 64, 64, 64],
  // 5-й ряд, 3 первых кадра (Плачущая) - Based on user's new request
  'scared_idle': [384, 128, 64, 64, 448, 128, 64, 64, 512, 128, 64, 64],
  // 6-й ряд, 3 последних кадра (Ангелочек)
  'gameover': [384, 320, 64, 64, 448, 320, 64, 64, 512, 320, 64, 64]
}
...
// Stops the main *game loop* and starts the *game over sprite*
watch(gameEnded, (hasEnded) => {
 if (hasEnded) {
  if (gameLoop) gameLoop.stop()
  
  // Allow Vue to render the v-if="gameEnded" layer
  setTimeout(() => {
   const sprite = (gameOverSpriteRef.value as any)?.getStage()
   if (sprite) {
    sprite.start()
   }
  }, 50)
