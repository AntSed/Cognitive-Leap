...
function initGame() {
    ...
    // Slower game speed for Advanced version
    const baseSpeedFactor = 0.8; 
    if (window.innerWidth < 768) {
        globalGameSpeedFactor = 0.5 * baseSpeedFactor; // 40% speed for mobile
    } else {
        globalGameSpeedFactor = baseSpeedFactor; // 80% speed for desktop
    }
...
function getRandomNonZeroUnit(min, max) {
    let num;
    do {
...
function generateProblem() {
    const problemType = getRandomInt(1, 4);
    let num1, num2, result, divisor;

    switch (problemType) {
        // Case 1: Addition/Subtraction within 150
        case 1:
            if (Math.random() < 0.5) { // Addition
                num1 = getRandomNonZeroUnit(11, 139);
                num2 = getRandomNonZeroUnit(11, 150 - num1);
                currentProblem.answer = num1 + num2;
                currentProblem.text = `${num1} + ${num2}`;
            } else { // Subtraction
                num1 = getRandomNonZeroUnit(30, 150);
                num2 = getRandomNonZeroUnit(11, num1 - 11);
                currentProblem.answer = num1 - num2;
                currentProblem.text = `${num1} - ${num2}`;
            }
            break;

        // Case 2: Full Multiplication/Division Table (with exclusions)
        case 2:
            if (Math.random() < 0.5) { // Multiplication
                do {
                    num1 = getRandomInt(2, 12);
                    num2 = getRandomInt(2, 12);
                } while (num1 === num2 || num1 === 2 || num2 === 2 || num1 === 10 || num2 === 10);
                currentProblem.answer = num1 * num2;
                currentProblem.text = `${num1} × ${num2}`;
            } else { // Division
                do {
                    result = getRandomInt(3, 12);
                    num2 = getRandomInt(3, 12);
                } while (result === num2 || result === 10 || num2 === 10);
                num1 = result * num2;
        ...
        // Case 3: Basic Equations (Addition/Subtraction)
        case 3:
            if (Math.random() < 0.5) { // ? + num2 = result
                result = getRandomNonZeroUnit(50, 150);
                num2 = getRandomNonZeroUnit(11, result - 20);
                currentProblem.answer = result - num2;
                currentProblem.text = `? + ${num2} = ${result}`;
            } else { // num1 - ? = result
                num1 = getRandomNonZeroUnit(50, 150);
                result = getRandomNonZeroUnit(11, num1 - 20);
                currentProblem.answer = num1 - result;
                currentProblem.text = `${num1} - ? = ${result}`;
            }
            break;

        // Case 4: Basic Equations (Multiplication/Division with exclusions)
        case 4:
            if (Math.random() < 0.5) { // ? * num2 = result
                do {
                    num2 = getRandomInt(3, 12);
                    result = getRandomInt(3, 12);
                } while (num2 === result || num2 === 10 || result === 10);
                currentProblem.answer = result;
                currentProblem.text = `? × ${num2} = ${result * num2}`;
            } else { // num1 / ? = result
                do {
                    divisor = getRandomInt(3, 12);
                    result = getRandomInt(3, 12);
                } while (divisor === result || divisor === 10 || result === 10);
                num1 = divisor * result;
                currentProblem.answer = divisor;
                currentProblem.text = `${num1} ÷ ? = ${result}`;
            }
            break;
    }
...
function spawnProblemWave() {
    generateProblem();
    // Conditionally add " = ?" only for non-equation problems
    problemDisplayRef.value.textContent = currentProblem.text.includes('?') 
        ? currentProblem.text 
        : currentProblem.text + " = ?";
    enemies = []; 
    bullets = [];
...
function update() {
    ...
    // Emit 'completed' event when score reaches 500
    if (!hasCompletedEmit && score >= 500) {
        emit('completed');
