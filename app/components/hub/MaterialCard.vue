  ...
  <div class="material-card" :class="`status-${material.status}`">
    ...
    <div class="card-footer">
      ...
      <div class="card-actions">
        <button v-if="canManageStatus" class="action-btn" @click="openStatusModal(material, allProgramLessons, updateTools)" :title="t('hub.card.manageStatusTooltip')">
        ...
        <button v-if="canUnpin" @click="openUnpinModal(material, selectedLessonId, updateTools)" class="action-btn unpin-btn" :title="t('hub.card.unpinTooltip')">
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244" />
          </svg>
        </button>
        <button v-if="canDelete" @click="openDeleteModal(material, updateTools)" class="action-btn delete-btn" :title="t('hub.card.deleteTooltip')">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
const props = defineProps({
  /**
   * The core learning material object this card represents.
   * @type {object}
   */
  material: { type: Object, required: true },
  /**
   * The profile of the currently logged-in user, used for permissions.
   * @type {object}
   */
  currentUser: { type: Object, required: true },
  /**
   * The ID of the currently selected lesson, if any.
   * @type {string | null}
   */
  selectedLessonId: { type: String, default: null },
  /**
   * An object containing granular functions for updating the parent UI state
   * (e.g., incrementing/decrementing counts, refreshing the material list).
   * @type {{ increment: (id: string) => void, decrement: (id: string) => void, refreshMaterials: () => Promise<void> }}
   */
  updateTools: { type: Object, required: true },
  /**
   * The complete, pre-fetched list of all lessons for the current program.
   * This is passed down to modals to prevent redundant data fetching.
   * @type {Array<object>}
   */
...
// --- HANDLERS ---
/**
 * Emits an event to the parent component to handle reordering.
 * @param {number} newPosition - The new position value from the EditablePosition component.
 */
...
/**
 * Handles inline editing of a field on the material.
 * Updates the database and then uses the refresh tool to update the UI.
 * @param {string} field - The name of the database column to update.
