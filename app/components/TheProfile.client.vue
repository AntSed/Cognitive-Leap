      ...
      <section class="mb-8">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-4">
              ...
              <button @click="handleLinkGoogleInProfile" class="w-full py-3 px-4 rounded-lg font-semibold text-white bg-[#4b5563] hover:bg-[#6b7280] flex items-center justify-center gap-2">
                  <svg class="w-5 h-5" viewBox="0 0 48 48"></svg>
...
  try {
    const { error } = await supabase.functions.invoke('delete-relation', { body: { student_id: relation.student_id, curator_id: relation.curator_id } });
    if (error) throw error;
    await fetchProfileAndRelations();
  } catch (error) {
...
    const { error } = await supabase.functions.invoke('invite-user', { body: { inviteeEmail: inviteEmail.value, inviterId: user.value.id, inviterName: inviterName, inviterLang: locale.value, inviteType: inviteType.value, relationType: relationType.value } });
    if (error) {
        alert(`Error: ${error.message}`);
    } else {
        closeModal();
...
// ==================================================================
// ИЗМЕНЕНИЕ ЗДЕСЬ: ЭТА ФУНКЦИЯ ТЕПЕРЬ ДЛЯ ВХОДА, А НЕ ДЛЯ ПРИВЯЗКИ
// ==================================================================
const handleGoogleSignIn = async () => {
  authMessage.value = '';
  // Используем signInWithOAuth, так как пользователь не залогинен
...
// Эта функция для привязки, она остается с linkIdentity
